...
async function getIntelligentSummaryFromText(text) {
    ...
    // PROMPT REFINADO COM NOVAS INSTRUÇÕES
    const prompt = `
    ...
    console.log("[INFO] Chamando API do Gemini para resumo inteligente refinado...");
    const result = await model.generateContent(prompt);
    const response = await result.response;
    const summaryText = response.text();
    console.log("[SUCCESS] Resumo refinado do Gemini recebido.");
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // AGORA, CHAMAMOS A IA COM O NOVO PROMPT REFINADO
